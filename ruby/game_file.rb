class Wordgame 
	...
	def word_selection
		puts "User one please select the word user two will be trying to guess"
		@word_choice = gets.chomp.downcase 
		@underscores = "_" * @word_choice.length
		@display = @underscores.chars
	end 
	
	def user_guess 
		puts "User two please guess a letter from User one's word selection"
